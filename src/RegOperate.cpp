...
{
	return reg_write_ex( key_handle, value_name, REG_SZ,(LPBYTE const)str, (int)strlen(str) + 1 );
}
...
REGOPERATE_IMPL_FUNC(void) reg_force_delete( LPCSTR key_name )
{
	...
	int i = 0;
	for( ; !RegEnumKey( key_handle, i, &key_buff[0], (DWORD)key_buff.size() ); i++ )
	{
